...
export default function MainWindow({ activeTab, onTabChange, data, loading }) {
  ...
  return (
    <div
      ...
      style={{
        ...
        width: '100%',
        maxWidth: '860px',
        minHeight: isMobile ? 'calc(100vh - 80px)' : '520px',
      }}
    >
      {/* Title bar */}
      <div className="win-titlebar" style={{ display: 'flex', alignItems: 'center', gap: '6px', flexShrink: 0 }}>
      ...
      {/* Menu bar — hidden on mobile to save space */}
      {!isMobile && <div
        style={{
          background: '#C0C0C0',
      ...
      >
        <AnimatePresence mode="wait">
          ...
          >
            {activeTab === 'about' && (
              <AboutTab profile={data.profile} loading={loading} isMobile={isMobile} />
            )}
            {activeTab === 'projects' && (
              <ProjectsTab projects={data.projects} loading={loading} isMobile={isMobile} />
            )}
            {activeTab === 'skills' && (
              <SkillsTab skills={data.skills} loading={loading} isMobile={isMobile} />
            )}
            {activeTab === 'experience' && (
              <ExperienceTab experience={data.experience} loading={loading} />
            )}
            {activeTab === 'contact' && (
              <ContactTab profile={data.profile} loading={loading} isMobile={isMobile} />
            )}
          </motion.div>
        </AnimatePresence>
      </div>
